// app.js — DS AI Chat-First Build "Loose Funny Mode + Clarify-Until-JSON"
// Rules:
// - GPT is the only brain. It should output { narration, ops?, choices? }.
// - Narration ALWAYS shows (funny/cynical). If ops are missing/malformed, we treat it
//   as a clarification turn and show: "💬 Clarification noted. No state change yet."
// - Only fatal cases (no payload or no narration at all) block the turn.
// - No 'OK' fallbacks anywhere. Preserve working features (sidebar, standings, race sim).
...
function loadLocal() {
  try {
    const raw = localStorage.getItem('ccsf_state'); if (!raw) return null;
    const state = JSON.parse(raw); if (!state?.meta?.season) return null;
    return state;
  } catch { return null; }
}
...
/* ----------------------- Clean raw reply for display ----------------------- */
function cleanReplyText(x) {
  if (!x) return '';
  const s = String(x).replace(/```[\s\S]*?```/g, '').trim();
...
function normalizePointsSystem(ps) {
  ...
  if (Array.isArray(ps)) {
    const obj = {}; for (let i = 0; i < ps.length; i++) obj[i+1] = Number(ps[i] || 0);
...
async function runRaceSim() {
  ...
  try {
    ...
    const systemPrompt =
      ...
      'Return JSON ONLY inside one object: { "narration": string, "ops": { "_type":"ccsf_ops_v1", "changes":[ ... ] } }.\n' +
      'Use the exact key name "narration". Do not rename it.\n' +
      'Narration: 3–5 short sentences, always funny, snarky, and cynical. Use paddock gossip and sarcasm. No serious tone. No code fences.\n' +
      ...
      'WRITE a PUSH to /stats/race_results with: { "round": ' + JSON.stringify(nxt?.round || 1) + ', "name": ' + JSON.stringify(nxt?.name || 'Unknown') + ', "finishers":[{"driver":"<roster>","team":"<team>","position":1,"points":<from table>}, ...] }.\n' +
    ...
    const res = await fetch('api/chat', {
      ...
      body: JSON.stringify({
        messages: [
          ...
          { role:'user',   content: 'Simulate qualifying and the race for the next event NOW. Return JSON object as specified.' }
        ]
      })
    });
    ...
    if (check.fatal === true) {
      ...
      const warn = `\n\n⚠️ ${check.reason || 'fatal-parse'} — no response available.`;
      thinkingEl.textContent = outText + warn;
    ...
    if (check.ops) {
      ...
      if (resOps.ok) {
        ...
        outText = (outText || '🧾 Update applied.') + `\n\n🧾 Database updated: ${resOps.changed.join(', ')}`;
      } else {
        outText = (outText || '🙃 No meaningful change.') + `\n\n(ℹ️ Ops had no effect)`;
      }
    } else {
      outText = (outText || '🤔 Clarify?') + `\n\n💬 Clarification noted. No state change yet.`;
    }
...
/* ----------------------- General chat (Loose Funny Brain) ----------------------- */
async function send() {
  ...
  try {
    ...
    const systemPrompt =
      'You are the ONLY brain of an F1 management sim. Return a single JSON object only: ' +
      '{"narration": string, "ops": {"_type":"ccsf_ops_v1","changes":[...]}, "choices":[{"id":"A|B|C","label":"...","reply":"..."}]?}.\n' +
      'Use the exact key name "narration". Do not rename it.\n' +
      'Narration: 3–5 short sentences, always funny, cynical, and snarky. Use paddock gossip, sarcasm, and deadpan humor. Never serious. No code fences.\n' +
