// app.js — DS AI Chat-First Build "Loose Funny Mode" (No Autopilot, No Fallbacks to 'OK')
// GPT is the only brain: it must always emit narration (funny/cynical), ops, and (if open-ended) exactly 3 choices.
// The client applies whatever ops are returned. If GPT returns bad/missing ops, nothing is applied—
// but narration is always shown with a clear warning (no more 'OK').
// Sidebar-safe layout; race sim strict JSON shape but tolerant extractor. Keep it simple.
...
async function runRaceSim() {
  ...
  try {
    ...
    let payload = null;
    payload = (typeof reply === 'object' && reply) ? reply : extractJsonBlock(String(reply ?? raw)) || null;

    const check = validateOpsPayload(payload);
    let outText = cleanNarration(payload?.narration || '🤡 (GPT forgot the punchline)');

    if (!check.ok) {
      thinkingEl.classList.remove('thinking');
      const warn = `\\n\\n⚠️ Malformed or missing ops (${check.reason}). No state change.`;
      thinkingEl.textContent = (outText && outText.trim().length ? outText : '🤡 (GPT sent vibes, not ops)') + warn;
