...
async function send() {
  ...
  try {
    ...
    const systemPrompt =
      ...
      'Narration: 3–5 vivid, cynical sentences that lead the scene; never reply with one word. No code fences.\n' +
      'If the user intent is open-ended (visit/talk/inspect/try things), ALWAYS include exactly 3 choices. ' +
      'Each choice ≤10 words and must include fields {"id":"A|B|C","label","reply"}.\n' +
      'OPS: Always emit ops. If the request is ambiguous, emit a small scene/event push to /events that summarizes the user’s intent. Use only set/inc/push. Car attrs 0–100.\n' +
